...
2. Bonus EDA function: `skimr::skim()`
...
some caveats about data (just a good example - not trying to model anything)

* predictors are inputs to arithmetic used to calculate the final score
...
something about using plot missing


### New usage (exedite EDA + modeling)

something about efficiency of combining 
* skimr, plot_missing, plot-bar and plot-historgram

to quickly make decisions about what to do w/data


#### WORKFLOW

1. show silly plot-missing
2. new eda workflow:
    * skimr 
    * add findings to preprocessiing workflow
    * plot-bar + make note of findings (for adding to pipe)
    * plot-bar + same as bar
3. conclude w/all findings listed that now can go into pipeline.
4. make simple recipe 
5. add recipe to pipeline
5. split training set
6. apply preprocessing to datasets (test/train)???
7. wrap up...
...
### Wrap up w/throwing all these findings into 'recipe pipeline'
