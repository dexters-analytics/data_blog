...
2. The `function` I use often: `janitor::clean_names()`.
...
2. Deliver a map of US Starbucks locations (by ownership type).
3. Tabulate the various types of Starbucks Ownership:
    * Worldwide & 
    * US (lower 48)
...
#readxl::read_xlsx("static/01_data/week6_coffee_chains.xlsx")
```
...
The idea is that as you conduct your Exploratory Data Analysis `(EDA)`, you will discover pre-processing +/or cleaning steps that need to be added to your pipeline. 
...
The `glimpse()` function allows us to quickly assess column names, data-types, and also view a sample of the values contained in each column - you can read more about the `glimpse()` function in my archive post, [Examining Data with glimpse()](https://www.exploringdata.org/post/examining-data-with-glimpse/){target="_blank"}.
...
`janitor::get_dupes()` is at the top of the list for newly discovered `functionality` within the `{janitor}` package.
...
Now that we've done our due diligence in being sure we've dealt with data issues, let's wrap up this analysis by compiling a map along with the requested tabulated data.
...
Now that our data has been cleaned up a bit, we can finally make that map.
...
However, `{janitor}` is packed full of other goodies - the creators have crafted a number of `adorn` options for formatting our outputs. I used the `adorn_totals` and `adorn_pct_formatting` to tidy up and make our table ready for presentation.
...
I've been learning Data Science at [Business Science University](http://bit.ly/2UbPat2){target="_blank"} where Matt Dancho (the owner) has been teaching me how to succeed - the courses helped me land a full-time gig at Sephora on the Analytics team (Thank you, Matt Dancho).

Join me on the journey. Check out this link to get 15% off of the courses that are helping me take my career in Analytics to the next level: [Business Science Courses in Data Science](http://bit.ly/2UbPat2){target="_blank"}
...
Enter your [Email Here](https://tinyletter.com/dexters-analytics){target="_blank"} to join the many other subscribers leveling up there skills here at `Exploring-Data`.<br/>  
