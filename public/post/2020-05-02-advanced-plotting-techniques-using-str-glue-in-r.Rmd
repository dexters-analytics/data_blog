...
Let's take a quick peak at the transposed table to inspect the columns and data types.

```{r}
sf_trees_data_raw_tbl %>% glimpse()
```
...
Here is the manipulated text that will be useful once we plot these data.

```{r}
redwood_labeled_text_tbl %>% select(label_text)
```
...
And here is our plot with our the labels engineered in the last few steps. And that's just one example of why I love `str_glue()` - Simply Awesome!
...
Stay tuned (+connected) for more posts on Wrangling + Exploring data (`programatically`).
